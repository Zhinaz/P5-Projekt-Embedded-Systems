...
The event in \lstref{lst:eventexample} is the only event in the \projname{}. It is used to trigger the alarm, seen in \lstref{lst:alarmexample}, at the beginning of the loop in the Main task. 
...
The alarm on \lstref{lst:alarmexample}, is used in triggering an action for the background processes of boundary detection and bluetooth connection. An alarm is set when the colour sensors are initialised. This is the only alarm used in \projname{}.
